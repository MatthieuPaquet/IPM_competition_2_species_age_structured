...
load(file="samples_coexistence_model_loglogit.Rdata")
load(file="simul_coexistence_model_loglogit.Rdata")
#parameter set
parameterset <- 1
...
log.N1astar <- log(35.1841)
log.N2astar <- log(19.20152)
...
if (PLOT){
  for (i in 1:n.simul) {
    plot(list.samples[[i]]$chain1[,'alphs[1, 1]'],type="l",ylim=c(-1,1))
    points(list.samples[[i]]$chain2[,'alphs[1, 1]'],type="l",col="red")
    abline(h=alphs[1,1])
    plot(list.samples[[i]]$chain1[,'alphs[2, 1]'],type="l",ylim=c(-1,1))
    points(list.samples[[i]]$chain2[,'alphs[2, 1]'],type="l",col="red")
    abline(h=alphs[2,1])
    plot(list.samples[[i]]$chain1[,'alphs[1, 2]'],type="l",ylim=c(-1,1))
    points(list.samples[[i]]$chain2[,'alphs[1, 2]'],type="l",col="red")
    abline(h=alphs[1,2])
    plot(list.samples[[i]]$chain1[,'alphs[2, 2]'],type="l",ylim=c(-1,1))
    points(list.samples[[i]]$chain2[,'alphs[2, 2]'],type="l",col="red")
    abline(h=alphs[2,2])
    plot(list.samples[[i]]$chain1[,'betas[1, 1]'],type="l",ylim=c(-1,1))
    points(list.samples[[i]]$chain2[,'betas[1, 1]'],type="l",col="red")
    abline(h=betas[1,1])
    plot(list.samples[[i]]$chain1[,'betas[2, 1]'],type="l",ylim=c(-1,1))
    points(list.samples[[i]]$chain2[,'betas[2, 1]'],type="l",col="red")
    abline(h=betas[2,1])
    plot(list.samples[[i]]$chain1[,'betas[1, 2]'],type="l",ylim=c(-1,1))
    points(list.samples[[i]]$chain2[,'betas[1, 2]'],type="l",col="red")
    abline(h=betas[1,2])
    plot(list.samples[[i]]$chain1[,'betas[2, 2]'],type="l",ylim=c(-1,1))
    points(list.samples[[i]]$chain2[,'betas[2, 2]'],type="l",col="red")
    abline(h=betas[2,2])
    plot(list.samples[[i]]$chain1[,'fert1'],type="l",ylim=c(0,3))
    points(list.samples[[i]]$chain2[,'fert1'],type="l",col="red")
    abline(h=fert1)
    plot(list.samples[[i]]$chain1[,'fert2'],type="l",ylim=c(0,3))
    points(list.samples[[i]]$chain2[,'fert2'],type="l",col="red")
    abline(h=fert2)
    plot(list.samples[[i]]$chain1[,'phi1'],type="l",ylim=c(-4,4))
    points(list.samples[[i]]$chain2[,'phi1'],type="l",col="red")
    abline(h=phi1)
    plot(list.samples[[i]]$chain1[,'phi2'],type="l",ylim=c(-4,4))
...
N1a <- seq(1,max(N1asimul),length=n.n) #density index adults species1
N2a <- seq(1,max(N2asimul),length=n.n) #density index adults species2
##to adjust
surv1a <- surv2a <-
  fec1 <- fec2 <- matrix(NA,n.n,n.n) 
for (i in 1:n.n) {
  surv1a[,i] <- plogis(phi1 + betas[1,1] * (log(N1a) - log.N1astar) + betas[1,2] * (log(N2a[i]) - log.N2astar))
  surv2a[,i] <- plogis(phi2 + betas[2,1] * (log(N1a) - log.N1astar) + betas[2,2] * (log(N2a[i]) - log.N2astar))
  fec1[,i] <- exp(fert1 + alphs[1,1] * (log(N1a) - log.N1astar) + alphs[1,2] * (log(N2a[i]) - log.N2astar))
  fec2[,i] <- exp(fert2 + alphs[2,1] * (log(N1a) - log.N1astar) + alphs[2,2] * (log(N2a[i]) - log.N2astar))
}#i
par(mfrow=c(2,2),mar=c(1.4, 0, 0, 0))
persp(z = surv1a,x=N1a,y=N2a,xlab="N1a",ylab="N2a",zlab="Survival species 1",
      zlim=c(0,0.2),theta = 60,ticktype="detailed")
...
persp(z = fec1,x=N1a,y=N2a,xlab="N1a",ylab="N2a",zlab="Fecundity species 1",
      zlim=c(0,10),theta = 60,ticktype="detailed")
